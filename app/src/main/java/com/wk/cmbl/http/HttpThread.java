...
public class HttpThread {
     ...
     */
    public void doRequestGet(final HttpBodyBase bodyBase, final Handler handler) {
        if (bodyBase == null) {
        ...
        if (!CMBLTools.isConnection(context)) {
            ...
            kjh.get(bodyBase.getUrl(), getCallBack(handler, bodyBase.getAction()));
        }
    }
	 ...
	 * @param handler 返回监听handler
	 */
	public void doRequest(final HttpBodyBase bodyBase, final Handler handler) {
		if (bodyBase == null) {
			Log.e(FinalValue.Http_Tag, "the HttpBodyBase is null");
			return;
		}
		...
		if (!CMBLTools.isConnection(context)) {
			...
			kjh.post(bodyBase.getUrl(), params, getCallBack(handler, bodyBase.getAction()));
		}
	}
